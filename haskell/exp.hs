...
-- Function to parse a pair from a string
parsePair :: String -> Maybe (Double, Double)
parsePair str = case reads str of
                  [(x, rest)] -> case reads rest of
                                   [(y, _)] -> Just (x, y)
                                   _        -> Nothing
                  _           -> Nothing

-- Read a list of pairs from the user as a Haskell expression and evaluate it to obtain Point2D list
readPoints :: IO [Point2D]
readPoints = do
    putStrLn "\nInserisci il dataset come espressione Haskell che valuti a una lista di coppie (x, y):"
    input <- getLine
    let tokens = words input
        maybePairs = mapM parsePair tokens
    case maybePairs of
        Just pairs -> return $ tupleToPoint2D pairs
        Nothing -> do
            putStrLn "Input non valido. Assicurati che l'espressione sia corretta."
            readPoints
